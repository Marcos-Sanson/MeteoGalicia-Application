...
def create_graph(df_reorganized, year, title_label):
    ...
    if y_axis_label == 'Chuvia':
        y_axis_label += ' (litros/m2)'  # Add unit for 'Chuvia'

    plt.ylabel(y_axis_label)  # Set y-axis label
    plt.title(f'{title_label} para el año {year}')  # Translate and set graph title
...
# Example usage
input_file_path = 'Número de días de helada_mensuales_test1/resultadoCSV_Columnas.csv'     # Replace with the path to your input ODS file
output_file_path = 'Número de días de helada_mensuales_test1/graph_Número de días de helada_mensuales.ods'     # Replace with the desired path for the output ODS file
...
# Create a graph for a selected year
year = 2011  # Replace with the desired year
create_graph(df_reorganized, year, title_label)
